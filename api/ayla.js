export default async function handler(req, res) {
  ...
  // Permite apenas POST
  if (req.method !== 'POST') {
    return res.status(400).json({ error: 'Método não permitido. Use POST.' });
  }
  ...
  try {
    ...
    return res.status(500).json({ error: 'Erro ao se conectar com o OpenRouter.', detalhe: error.message });
  }
}
