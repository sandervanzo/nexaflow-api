export default async function handler(req, res) {
    ...
    let body = {};
    try {
      // Vercel já faz o parsing automático em req.body se configurado corretamente,
      // mas aqui garantimos o fallback
      body = typeof req.body === 'string' ? JSON.parse(req.body) : req.body;
    } catch (error) {
      return res.status(400).json({ error: 'Erro ao interpretar o corpo da requisição.' });
    }
    ...
    try {
      ...
      return res.status(500).json({ error: 'Erro ao se comunicar com o OpenRouter', details: err.message });
    }
  }
